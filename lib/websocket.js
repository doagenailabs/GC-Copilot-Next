...
export async function initializeWebSocket() {
    ...
    console.log("GCCopilotNext - websocket.js - About to call postNotificationsChannels...");

    try {
        const response = await apiInstance.postNotificationsChannels({});
        console.log("GCCopilotNext - websocket.js - Received response from postNotificationsChannels:", response);

        channelId = response.id;
        const ws = new WebSocket(response.connectUri);

        ws.onmessage = async (event) => {
            console.log("GCCopilotNext - websocket.js - WebSocket message received:", event.data);
            ...
            if (data.eventBody.disconnectType) {
                console.log("GCCopilotNext - websocket.js - Received disconnectType in WebSocket message. Closing WebSocket.");
                ws.close();
            }
        };

        ws.onopen = () => {
            console.log("GCCopilotNext - websocket.js - WebSocket connection opened.");
            subscribeToTopic(channelId, 'v2.conversations.' + window.conversationId + '.transcription');
        };

        ws.onerror = (error) => {
            console.log("GCCopilotNext - websocket.js - WebSocket encountered an error:", error);
        };

        ws.onclose = (event) => {
            console.log("GCCopilotNext - websocket.js - WebSocket connection closed:", event);
        };
    } catch (err) {
...
function subscribeToTopic(channelId, topicName) {
    console.log("GCCopilotNext - websocket.js - Attempting to subscribe to topic:", topicName);
    ...
    apiInstance.postNotificationsChannelSubscriptions(channelId, [topicName])
        .then(() => {
            console.log("GCCopilotNext - websocket.js - Successfully subscribed to topic:", topicName);
        })
        .catch(err => {
