...
export async function initializeWebSocket(platformClient) {
    debug('Initializing WebSocket');
    
    if (!platformClient) {
        error('Platform client not provided');
        throw new Error('Platform client not provided');
    }
    ...
    let channelId;
    debug('Using platformClient from context');
    ...
    try {
        ...
        wsInstance = setupWebSocket(response.connectUri, channelId, platformClient);
        return wsInstance;
    } catch (err) {
        error('Failed to initialize WebSocket:', err);
        handleWebSocketError(err, platformClient);
        throw err;
    }
}

function setupWebSocket(connectUri, channelId, platformClient) {
    ...
    ws.onerror = (event) => {
        ...
        handleWebSocketError(new Error('WebSocket encountered an error'), platformClient);
    };

    ws.onclose = (event) => {
        log('WebSocket connection closed:', event);
        handleWebSocketClose(event, platformClient);
        wsInstance = null;
    };
...
async function processTranscription(data) {
    try {
        ...
        // Clean old transcripts (keep last 30 seconds)
...
function handleWebSocketClose(event, platformClient) {
    if (event.code !== 1000) {
        error(`WebSocket closed abnormally with code ${event.code}`);
        handleWebSocketError(new Error(`Abnormal WebSocket closure with code ${event.code}`), platformClient);
    } else {
