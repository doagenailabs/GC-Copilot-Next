export function startGCSDKs(clientId) {
  ...
  function assignConfiguration() {
    ...
    if (searchParams.has(qParamConversationId)) {
      ...
      console.log('gcSDKs.js - GCCopilotNext - Conversation ID set from URL parameter:', conversationId);
    }
    ...
    if (searchParams.has(qParamLanguage)) {
      ...
      console.log('gcSDKs.js - GCCopilotNext - Language set from URL parameter:', language);
    } else {
      let local_lang = localStorage.getItem(`${appName}_language`);
      if (local_lang) {
        language = local_lang;
        console.log('gcSDKs.js - GCCopilotNext - Language set from localStorage:', language);
      }
    }
    ...
    if (searchParams.has(qParamGcHostOrigin)) {
      ...
      console.log('gcSDKs.js - GCCopilotNext - gcHostOrigin set from URL parameter:', gcHostOrigin);
      return gcHostOrigin;
    } else {
      let local_gcHostOrigin = localStorage.getItem(`${appName}_gcHostOrigin`);
      if (local_gcHostOrigin) {
        gcHostOrigin = local_gcHostOrigin;
        console.log('gcSDKs.js - GCCopilotNext - gcHostOrigin set from localStorage:', gcHostOrigin);
        return gcHostOrigin;
      }
      console.log('gcSDKs.js - GCCopilotNext - gcHostOrigin not found in both URL parameters and localStorage.');
      return null;
    }
  }
}
