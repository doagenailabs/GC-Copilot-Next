export function startGCSDKs(clientId) {
  return new Promise(async (resolve, reject) => {
    ...
    // Access the SDKs from the global window object
    let platformClient, ClientApp;
    try {
      console.log('GCCopilotNext - gcSDKs.js - Accessing Genesys Cloud SDKs from window');
      platformClient = window.platformClient;
      ClientApp = window.purecloud.apps.ClientApp;

      if (!platformClient || !ClientApp) {
        throw new Error('Genesys Cloud SDKs not loaded');
      }

      console.log('GCCopilotNext - gcSDKs.js - Successfully accessed Genesys Cloud SDKs');
    } catch (e) {
      console.error('GCCopilotNext - gcSDKs.js - Error accessing SDK modules:', e);
      reject(e);
      return;
    }
    ...
    try {
      ...
      // Expose platformClient and myClientApp on window for other modules if needed
    ...
    client
      ...
      .catch((err) => {
        console.error('GCCopilotNext - gcSDKs.js - Error during loginImplicitGrant:', err);
        reject(err);
      });
  });
}
