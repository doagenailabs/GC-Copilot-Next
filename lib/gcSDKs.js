...
export function startGCSDKs(clientId) {
  return new Promise((resolve, reject) => {
    ...
    // Debug global scope availability
    debug('Window globals state:', {
      hasPlatformClient: typeof window.platformClient !== 'undefined',
      hasPurecloud: typeof window.purecloud !== 'undefined',
      hasRequire: typeof window.require !== 'undefined',
      windowKeys: Object.keys(window),
      documentReadyState: document.readyState
    });

    // Debug script loading status
    const scripts = document.querySelectorAll('script');
    debug('Loaded scripts:', Array.from(scripts).map(s => ({
      src: s.src,
      loaded: s.complete,
      async: s.async,
      defer: s.defer
    ...
    try {
      ...
      // Fallback to window.platformClient if require didn't work
      ...
      if (!platformClient) {
        debug('Platform client availability check:', {
          windowPlatformClient: !!window.platformClient,
          windowRequire: typeof window.require === 'function',
          windowPurecloudsApps: !!window.purecloud?.apps,
          requireError: true
        });
        throw new Error('Unable to initialize platformClient through any method');
      }

      debug('Got platformClient, checking for ApiClient');
      if (!platformClient.ApiClient) {
        debug('platformClient contents:', Object.keys(platformClient));
        throw new Error('platformClient.ApiClient not available');
      }

      const client = platformClient.ApiClient.instance;
      const usersApi = new platformClient.UsersApi();

      debug('Starting ClientApp initialization');
      const ClientApp = window.purecloud.apps.ClientApp;
      debug('ClientApp config:', {
