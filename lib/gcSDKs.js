...
// Wait for SDKs to be fully loaded with event handling
function waitForSDKsLoaded() {
  return new Promise((resolve) => {
    if (window.pcSDKLoaded) {
      debug('SDKs already loaded');
      resolve();
      return;
    }

    debug('Waiting for sdksLoaded event...');
    window.addEventListener('sdksLoaded', () => {
      debug('Received sdksLoaded event');
      resolve();
    }, { once: true });
  });
}
...
export async function startGCSDKs(clientId) {
  ...
  try {
    // First wait for SDKs to be loaded by SDKLoader
    debug('Waiting for SDKs to be loaded...');
    await waitForSDKsLoaded();
    debug('SDKs loading confirmed');

    // Wait for both SDKs with a longer timeout
    debug('Waiting for SDKs to initialize...');
...
export { dumpState, getUrlParameters };
