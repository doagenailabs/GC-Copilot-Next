...
export function startGCSDKs(clientId) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // Initialize ClientApp with environment from gcTargetEnv
      const searchParams = new URLSearchParams(window.location.search);
      const targetEnv = searchParams.get(PARAM_ENVIRONMENT);
      debug('Target environment from URL:', targetEnv);
      ...
      window.myClientApp = new ClientApp({
        ...
        gcEnvironment: targetEnv || environment // Use targetEnv if available, fallback to environment
      });

      debug('ClientApp initialized with:', {
        pcEnvironment: environment,
        pcOrigin: gcHostOrigin,
        gcEnvironment: targetEnv || environment
      });
    ...
    function assignConfiguration() {
      ...
      // Try to get gcHostOrigin from URL first, then localStorage
