...
// URL parameter extraction with fallback mechanisms
...
// Wait for SDK with enhanced error handling
  ...
  return new Promise((resolve, reject) => {
    ...
    const check = () => {
      ...
      if (window.purecloud?.apps?.ClientApp) {
        ...
        reject(new Error(`SDK not loaded after ${attempts} attempts (${elapsedTime}ms). Current state: ${JSON.stringify(state)}`));
      } else {
        if (!window.purecloud) {
...
export async function startGCSDKs(clientId) {
  ...
  try {
    ...
    log('Parameters for initialization:', params);
    ...
    // Ensure window.history.state is an object
    ...
    log('Redirect URI for authentication:', redirectUri);
