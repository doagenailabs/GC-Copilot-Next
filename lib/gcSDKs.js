...
export function startGCSDKs(clientId) {
  return new Promise((resolve, reject) => {
    ...
    // Initialize Client App with query param configs
    try {
      ...
      error('Failed to initialize ClientApp:', err);
      reject(err);
      return;
    }
    ...
    const waitForPlatformClient = () => {
      if (window.platformClient) {
        ...
        initializePlatformClient(window.platformClient);
    ...
    const initializePlatformClient = (platformClient) => {
      try {
        ...
        error('Failed to initialize platform client:', err);
        reject(err);
      }
    };
