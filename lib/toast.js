...
export function showToast(message, toastId, duration = 5000) {
  ...
  console.log("GCCopilotNext - toast.js - showToast - Is Salesforce environment:", isSalesforce);

  if (isSalesforce) {
    console.log("GCCopilotNext - toast.js - showToast - Creating custom toast for Salesforce LWC or other environments different than the native GC UI.");
    // Create a custom toast for Salesforce LWC
    createCustomToast(message, toastId, duration);
  } else {
    console.log("GCCopilotNext - toast.js - showToast - Using clientApp SDK toast logic when not embedded in Salesforce.");
